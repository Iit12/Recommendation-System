...
export const collectController = {
  /**
   * GET /api/v1/collect/search?q=<query>&platforms=<optional_comma_separated_list>
   */
  async searchListings(req, res, next) {
    try {
      ...
      // 4. Run collection service
      const result = await collectionService.collectListings(cleanQuery, requestedPlatforms);

      return sendSuccess(res, result.listings, {
        query: result.query,
