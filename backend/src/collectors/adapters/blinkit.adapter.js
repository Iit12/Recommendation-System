...
export class BlinkitAdapter extends BaseAdapter {
  constructor() {
    super('Blinkit', 'blinkit');
  }

  /**
   * Search Blinkit Quick Commerce listings (Phase 3 Controlled Mock Adapter)
   * Designed to be replaced with live API / connector in future phases.
   */
  async search(query) {
    const cleanQuery = query.toLowerCase().trim();
    const queryTokens = cleanQuery.split(/\s+/).filter(Boolean);
    const platformListings = MOCK_RAW_LISTINGS.blinkit || [];
